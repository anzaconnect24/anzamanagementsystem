...
import {
  ...
  sendProgramApplicationDocument,
} from "@/controllers/program_application_controller";
...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    getProgram(uuid).then((data) => {
      setProgram(data);
      setData(
        data.ProgramRequirements.map((item) => {
          ...
          <div className="py-6 px-4 md:px-6 xl:px-7.5">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                setloading(true);
                sendProgramApplication({ program_uuid: uuid }).then(
                  (response) => {
                    data.forEach(async (item) => {
                      await sendProgramApplicationDocument(
                        response.uuid,
                        item
                      ).then((data) => {});
                    });
            ...
            >
              <div className="space-y-3 py-8">
                {program.ProgramRequirements.map((item, index) => {
                  return (
                    <div key={index} className="space-y-1 flex">
              ...
              <div>
                <button
                  type="submit"
                  className="py-3 mt-5 px-4 flex justify-center bg-primary cursor-pointer text-white rounded hover:opacity-95"
                >
