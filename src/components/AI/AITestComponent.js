...
const AITestComponent = () => {
  ...
  const runFullAnalysisTest = async () => {
    ...
    try {
      ...
      const result = await analyzeCompleteReport(
        sampleReportData,
        sampleScoreData,
        sampleBusinessInfo,
      );
      ...
      <div className="p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            AI testing tools are restricted to Administrators only.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg">
      <h3 className="text-xl font-bold mb-6 text-gray-800">
        🤖 AI Service Testing Panel (Admin Only)
      </h3>
      ...
      <div className="mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2 bg-[#082d77] text-white rounded hover:bg-[#082d77] disabled:opacity-50 transition-colors"
        >
        ...
        {testResult && (
          ...
          >
            ...
            <p
              className={`mt-2 ${testResult.success ? "text-green-700" : "text-red-700"}`}
            >
      ...
      <div className="mb-6">
        ...
        {analysisResult && (
          <div
            className={`mt-4 p-4 rounded max-h-96 overflow-y-auto ${analysisResult.success ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
          >
            <h5
              className={`font-semibold ${analysisResult.success ? "text-green-800" : "text-red-800"}`}
            >
            ...
            {analysisResult.success ? (
              <div className="mt-2 text-green-700 text-sm">
                <p>
                  <strong>Executive Summary Generated:</strong>{" "}
                  {analysisResult.data.executiveSummary ? "Yes" : "No"}
                </p>
                <p>
                  <strong>Investment Thesis:</strong>{" "}
                  {analysisResult.data.investmentThesis ? "Yes" : "No"}
                </p>
                <p>
                  <strong>Recommendations:</strong>{" "}
                  {analysisResult.data.recommendations?.length || 0} items
                </p>
                <p>
                  <strong>Analysis Length:</strong>{" "}
                  {JSON.stringify(analysisResult.data).length} characters
                </p>
              ...
              <p className="mt-2 text-red-700">{analysisResult.error}</p>
            )}
          </div>
        )}
      </div>
