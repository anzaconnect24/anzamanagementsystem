...
import {getReviewerProgramApplications} from "@/app/controllers/program_application_review_controller"
...
const Page = () => {
  ...
  useEffect(() => {
        getReviewerProgramApplications(1,5).then((body)=>{
          setloading(false)
...
<div className="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
      ...
      {applications.map((item, key) => (
        ...
        >
          ...
          <div className="col-span-2 flex items-center">
            <p className="text-sm text-black dark:text-white">{item.ProgramApplication.User.name}</p>
          </div>
          <div className="col-span-2 flex items-center">
            <p className="text-sm text-black dark:text-white">{item.ProgramApplication.User.phone}</p>
          </div>
          <div className="col-span-1 flex items-center">
          <div onClick={()=>{
                  ...
                  setSelectedBusiness(null)
                   ...
                   Options
                   <div className={`absolute z-9 z-9 transition-all 
                   ...
                   -translate-x-4 bg-white shadow-lg   left-0 w-40 space-y-2 rounded-lg py-2 px-4 top-10`}>
                    {[
                      {title:"View application",path:`/viewProgramApplication/${item.ProgramApplication.uuid}`},
