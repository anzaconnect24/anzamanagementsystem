"use client"
import { getProgramApplication, updateProgramApplication } from "@/app/controllers/program_application_controller";
...
<div className="flex" >
          ...
          <div className="w-8/12 text-black">
              <div className="grid grid-cols-3 gap-3">
                {application.ProgramApplicationDocuments.map((item)=>{
                  return <a href={item.fileLink} target="_blank" className="w-full py-4 cursor-pointer text-center flex flex-col items-center justify-center  px-4 border border-stroke rounded hover:shadow">
      ...
      
       {
        ...
        <Link href="" onClick={()=>{
              updateProgramApplication(uuid,{status:"accepted"}).then(()=>{
                router.back()
              })
        }} className="bg-primary py-3 rounded px-4  hover:opacity-95 text-white">Accept</Link>
        <Link href={`/programApplicationRejection/${uuid}`} className="bg-danger py-3 rounded hover:opacity-95
