        ...
        <div className="rounded-lg border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="py-6 px-4 md:px-6 xl:px-7.5 flex justify-between ">
            ...
            {["Admin", "Enterprenuer"].includes(userDetails.role) && (
              <Link
                href="/newStory"
                className="py-2 px-3 bg-primary text-white rounded hover:opacity-95 cursor-pointer"
              >
                New story
              </Link>
            )}
          ...
          {users.length < 1 ? (
            ...
            <div>
              <div className="grid gap-4 grid-cols-3 px-8">
                {users.map((item, key) => (
                  <Link
                    className="w-full border p-4 rounded border-black border-opacity-20 hover:shadow-lg"
                    href={`/readStory/${item.uuid}`}
                    key={key}
                  >
                    <Image
                      alt=""
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <div>
                        ...
                        <div className=" text-bodydark2 text-sm italic">
