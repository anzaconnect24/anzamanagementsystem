          ...
          {users.length < 1 ? (
            ...
            <div className="pb-8">
              <div className="grid grid-cols-4 gap-4 px-6 py-2 ">
                {users.map((item, key) => (
                  <div
                    className="flex flex-col items-center justify-center border hover:scale-105 transition-all duration-200  border-bodydark border-opacity-70 py-12 rounded-lg "
                    key={key}
                  >
                    <Link
                      href={`businessDetails/${item.Business.uuid}`}
                      className=" bg-gray h-24 w-24 rounded-full"
                    >
                      <Image
                        height={2000}
                        width={2000}
