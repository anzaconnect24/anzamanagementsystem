...
const Page = () => {
  ...
  useEffect(() => {
        getReviewerInvestmentRequests(1,5).then((body)=>setRequests(body.data))
  }, []);
    return (
    <div>
      ...
      {requests.map((item, key) => (
        ...
        >
          ...
          <div className="col-span-2 flex items-center">
            <p className="text-sm text-black dark:text-white">{item.BusinessInvestmentRequest.User.phone}</p>
          </div>
          <div className="col-span-2 flex items-center">
            <p className="text-sm text-black dark:text-white">{item.BusinessInvestmentRequest.User.email}</p>
          </div>
          <div className="col-span-1 flex items-center">
          <div onClick={()=>{
