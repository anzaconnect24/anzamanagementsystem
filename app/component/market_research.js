...
const MarketResearch = ({business}) => {
    const [uploadingDocument, setuploadingDocument] = useState(false);
    const {userDetails,setUserDetails} = useContext(UserContext)
    return ( <div className="py-6 px-8">
        <div className="font-bold text-2xl text-black mb-4">Market research</div>
        <form onSubmit={(e)=>{
           e.preventDefault()
           const data = {
            title:e.target.title.value,
            documentName:"marketResearch",
          ...
          <input required disabled defaultValue="Market research" name="title" className="border-stroke w-full rounded" placeholder="Enter title"/> 
          </div>
          <div>
          <label className="mb-2.5 block font-medium text-black dark:text-white">
        ...
        {userDetails.Business.marketResearch && <div className="py-4 mt-8 px-4 shadow border-stroke ring-stroke rounded ring-1 flex flex-col justify-center text-lg font-bold items-center">
         ...
         stroke="currentColor" class="w-15 h-15">
          ...
          <div className="flex space-x-2">
             <div onClick={()=>{
                ...
                <h1 onClick={()=>{
          updateBusiness({marketResearch:null},userDetails.Business.uuid)
            let newData = userDetails;
